...
class VsphereConnection(BaseVsphereHandler):
    ...
    def get_about(self, disconnect=False):
        ...
        try:
            ...
            msg = _("Got a {c} on requesting 'about' information from vSphere {url}: {e}").format(
            ...
            if self.verbose > 1:
                ...
                LOG.debug(
                    _(
                        ...
                        cpu_l=cpus_label,
                        thr=cluster.cpu_threads,
                        t_l=thr_label,
                        mem=cluster.mem_gb_total,
                        net=len(cluster.networks),
                        nw_l=nw_label,
                        ds=len(cluster.datastores),
                        ds_l=ds_label,
                    )
                )
    ...
    def get_networks(self, vsphere_name=None, disconnect=False):
        ...
        if self.networks:
            msg = ngettext(
                "Found one Virtual Network.", "Found {n} Virtual Networks.", len(self.networks)
            )
    ...
    def get_hosts(self, re_name=None, vsphere_name=None, disconnect=False):
        ...
        if re_name is not None:
            if not hasattr(re_name, "match"):
                ...
                raise TypeError(msg)
            LOG.debug(
                _("Trying to get all host systems from vSphere with name pattern {!r} ...").format(
                    re_name.pattern
                )
            )
        else:
    ...
    def _get_hosts(
        ...
        if isinstance(child, (vim.ClusterComputeResource, vim.ComputeResource)):
            ...
            for host_def in child.host:
                ...
                LOG.debug(
                    _("Found host {h!r} in cluster {c!r}.").format(h=hostname, c=cluster_name)
                )
    ...
    def get_vms(
        ...
        LOG.debug(
            _("Trying to get list of VMs with name pattern {!r} ...").format(re_name.pattern)
        )
        vm_list = []
        if as_obj:
    ...
    def get_vm_folder(self, folder, disconnect=False):
        ...
        try:
            ...
            for part in parts:
                ...
                if self.verbose > 1:
                    LOG.debug(
                        _("Checking single VM folder {i}: {f!r}.").format(i=index, f=abs_path)
                    )
                ...
                for child in parent_folder.childEntity:
                    ...
                    if self.verbose > 1:
                        LOG.debug(
                            _("Found VM folder {n}, parent: {p}").format(
    ...
    def wait_for_tasks(
        ...
        """Wait for finishing the given vSphere task."""
        ...
        try:
            ...
            if max_wait:
                LOG.debug(
                    _("Waiting at most {m} seconds for tasks {t} to finish ...").format(
    ...
    def generate_vm_create_spec(
        ...
        ram_mb=1024,
        num_cpus=1,
        ds_with_timestamp=False,
        os_version=DEFAULT_OS_VERSION,
        cfg_version=DEFAULT_VM_CFG_VERSION,
    ...
    def generate_disk_spec(self, datastore_path, disks=None, disk_ctrl_type=None):
        ...
        LOG.debug(
            _("Using a {name!r} disk controller ({desc}).").format(name=ctrl_name, desc=ctrl_desc)
        )
        ...
        for size in disk_sizes2create:
            ...
            if self.verbose > 1:
                ...
                LOG.debug(
                    _("Adding spec for disk {n!r} with {gb} GiB => {kb} KiByte.").format(
    ...
    def _generate_if_create_spec(self, interface, dev_name):
        ...
        if self.verbose > 1:
            LOG.debug(
                _(
                    ...
                    d=dev_name,
                    n=interface.network_name,
                    m=interface.mac_address,
                    s=interface.summary,
                )
            )
        ...
        if interface.network_name in self.dv_portgroups:
            ...
            msg = _(
                ...
                "Virtual Network for network name {!r}."
            ).format(interface.network_name)
            LOG.error(msg)
            return None
    ...
    def set_mac_of_nic(self, vm, new_mac, nic_nr=0):
        ...
        if not virtual_nic_device:
            msg = _(
                "Did not found virtual ethernet device No. {no} (" "found {count} devices)."
            ).format(no=nic_nr, count=i)
            raise HandlerError(msg)
        ...
        LOG.debug(
            _("Successful changed MAC address of VM {v!r} to {m!r}.").format(v=vm_name, m=new_mac)
        )
    ...
    def custom_field_name(self, key_id):
        ...
        if self.custom_fields is None:
            ...
            try:
                ...
                msg = _(
                    "Got a {c} on requesting custom field names from vSphere {url}: {e}"
                ).format(c=e.__class__.__name__, url=self.connect_info.url, e=e)
                raise VSphereExpectedError(msg)
